 ...
 class EventosController extends Controller
{
    ...
    // }
        public function index()
    {
        // Obtener usuarios dependiendo del rol para la seleccion de jefe inmediato
        $rol = Auth::user()->id_rol;
        if ($rol == 1) {
            $usuario = User::whereIn('id_rol', [2, 3, 4, 5])->where('estado', 1)->get();
        } elseif ($rol == 2) {
            $usuario = User::whereIn('id_rol', [2, 5])->where('estado', 1)->get();
        } elseif ($rol == 3) {
            $usuario = User::where('id_rol', 2)->where('estado', 1)->get();
        } elseif ($rol == 4) {
    $usuario = User::where('estado', 1)
        ->where(function ($q) {
            $q->where(function ($qq) {
                $qq->where('id_rol', 3)->where('id_unidad', 7);
            })->orWhere(function ($qq) {
                $qq->where('id_rol', 2)->where('id_unidad', 8);
            })->orWhere(function ($qq) {
                $qq->where('id_unidad', 7); // <-- NUEVA CONDICIÓN adicional
            });
        })
        ->get();
} elseif ($rol == 5) {
            $usuario = User::where('id_rol', 5)->where('estado', 1)->get();
        } else {
            $usuario = collect();
        }
        $events = DB::table('eventos')->get();
        ...
        return view('calendario.index', ['eventos' => $events, 'usuarios' => $usuarios, 'unidad' => $unidad, 'roles' => $roles, 'usuario' => $usuario]);
    }
