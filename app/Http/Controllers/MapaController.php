...
//Para eliminar los links
    public function eliminarlink(Request $request)
        {
     ...
     * @return
     */
    public function modificar(Request $request, string $titulo){
        $request->validate(['descripcion' => 'nullable|string']);
        $desc = trim($request->input('descripcion'));

        // Soporte por si la columna se llama 'título' (con tilde)
        $col = DB::getSchemaBuilder()->hasColumn('noticias_2','titulo') ? 'titulo' : 'título';

        // upsert simple
        $affected = DB::table('noticias_2')->where($col, $titulo)->update([
            'descripcion' => $desc !== '' ? $desc : null, // si está vacío, guarda NULL
            'updated_at'  => now(),
        ]);

        if ($affected === 0) {
            DB::table('noticias_2')->insert([
                $col         => $titulo,
                'descripcion'=> $desc,
                'created_at' => now(),
                'updated_at' => now(),
    ...
    // log plano (igual al tuyo)
    date_default_timezone_set("America/Bogota");
    $ip  = $request->ip() ?? 'Desconocida';
    $msg = "Noticias2/Modificacion: ".Auth::user()->nombre.", ".Auth::user()->cedula
         . ", IP: {$ip}, ".date("d/m/Y H:i")." | {$col}={$titulo}".PHP_EOL;
    file_put_contents(base_path()."/storage/logs/informacion.log", $msg, FILE_APPEND);

    // si usas cache remember() al leer, olvida la clave:
    // Cache::forget($titulo);
