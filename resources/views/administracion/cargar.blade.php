...
<main id="main" class="main">
    ...
    <div class="container mx-auto tab-pane fade shadow rounded bg-white show active p-5 mt-4">
        ...
        <div class="container mx-auto tab-pane shadow rounded bg-white p-5 mt-4">
            ...
            <div class="tab-pane shadow-lg rounded bg-white p-5 mt-3"  style="height: 620px; text-align: justify; text-justify: auto; border:2px solid #f37531b4">
                ...
                <form class="formulario" action="{{route('noticias2.updateTexto', 'notifinleco')}}" method="post">
                    @csrf
                ...
                <form class="formulario" action="{{route('noticias2.updateTexto', 'notifinleco2')}}" method="post">
                    @csrf
                    <div class="row align-items-center">
                        <div class="col-10">
                            <p>Texto 2</p>
                ...
                <form class="formulario" action="{{route('noticias2.updateTexto', 'notifinleco3')}}" method="post">
                    @csrf
                    <div class="row align-items-center">
                        <div class="col-10">
                            <p>Texto 3</p>
    ...
    @if (Auth::user()->id_unidad == 1 || Auth::user()->id_rol == 3 || Auth::user()->id_unidad == 7 || Auth::user()->id_unidad == 26)
        ...
        @endforeach --}}
        @elseif((Auth::user()->id_unidad == 2) || (Auth::user()->id_rol == 2 && Auth::user()->id_unidad == 8 || Auth::user()->id_unidad == 26))
        <div class="container mx-auto tab-pane shadow rounded bg-white p-5 mt-4">
            <h2 class="head fw-bold mb-5">Agregar Links</h2>
            <p hidden>Clicks: <a id="clicks">0</a></p>
            <a class="text-primary text-decoration-none" id="nuevo-link" onClick="countClick()">
                                ...
                                <option disabled selected value=''>Seleccione la campaña</option>
                                    @foreach ($campañas as $campaña)
                                        @if ($campaña->campaña == 'Gestion Administrativa')
                                            <option value='{{ $campaña->id }}'>{{ $campaña->campaña }}</option>
                                        @endif
                                        @if (Auth::user()->id_rol == 2 && Auth::user()->id_unidad == 8)
                                            @if ($campaña->campaña == 'Gestion Documental')
                                                <option value="{{ $campaña->id }}">{{ $campaña->campaña }}</option>
                                            @endif
                                        @endif
                                    @endforeach
...
{{-- Eliminar Links --}}
        ...
        @if (Auth::user()->id_unidad == 1 || Auth::user()->id_rol == 3 || Auth::user()->id_unidad == 7 || Auth::user()->id_unidad == 2 || Auth::user()->id_rol == 2)
    <meta name="csrf-token" content="{{ csrf_token() }}" />
    <div class="container mx-auto tab-pane shadow rounded bg-white p-5 mt-4">
    ...
    {{-- registro de usuarios nuevos --}}
        @if (Auth::user()->id_unidad == 4 || Auth::user()->id_unidad == 1 || Auth::user()->id_unidad == 26)
                        ...
                        <div class="input-field">
                            ...
                            <select name="id_unidad" id="choice2">
                                ...
                                @foreach ($unidades as $unidad)
                                    ...
                                    @endif
                                    @if ($unidad->id == 8 || $unidad->id >= 10 && $unidad->id <= 18 || $unidad->id == 22 || $unidad->id == 23 || $unidad->id == 25 || $unidad->id == 27)
...
<script>
    const linksAgrupados = @json($linksAgrupados); // pasamos todos los links agrupados por campaña_id desde PHP a JS

    document.getElementById('campañaSelect').addEventListener('change', function () {
        const campañaId = this.value;
        const linkSelect = document.getElementById('linkSelect');

        // Limpiar el select de links
        linkSelect.innerHTML = '<option disabled selected value="">Seleccione un link</option>';

        // Si hay links para esta campaña
        if (linksAgrupados[campañaId]) {
            linksAgrupados[campañaId].forEach(link => {
                const option = document.createElement('option');
                option.value = link.id;
                option.text = link.nombre;
